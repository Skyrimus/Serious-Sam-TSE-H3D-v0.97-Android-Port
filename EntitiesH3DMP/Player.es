...
#include "EntitiesH3DMP/PlayerView.h"
#include "EntitiesH3DMP/MovingBrush.h"
#include "EntitiesH3DMP/Switch.h"
#include "EntitiesH3DMP/MessageHolder.h"
#include "EntitiesH3DMP/Camera.h"
#include "EntitiesH3DMP/WorldLink.h"
#include "EntitiesH3DMP/HealthItem.h"
#include "EntitiesH3DMP/ArmorItem.h"
#include "EntitiesH3DMP/WeaponItem.h"
#include "EntitiesH3DMP/AmmoItem.h"
#include "EntitiesH3DMP/PowerUpItem.h"
#include "EntitiesH3DMP/MessageItem.h"
#include "EntitiesH3DMP/AmmoPack.h"
#include "EntitiesH3DMP/KeyItem.h"
#include "EntitiesH3DMP/MusicHolder.h"
#include "EntitiesH3DMP/EnemyBase.h"
#include "EntitiesH3DMP/EnemyCounter.h"
#include "EntitiesH3DMP/PlayerActionMarker.h"
#include "EntitiesH3DMP/BasicEffects.h"
#include "EntitiesH3DMP/BackgroundViewer.h"
#include "EntitiesH3DMP/WorldSettingsController.h"
#include "EntitiesH3DMP/ScrollHolder.h"
#include "EntitiesH3DMP/TextFXHolder.h"
  ...
  void UpdateHUD() {
    ...
    for (i = 37; i <= 47; i++) {    CAttachmentModelObject* amo = m_moH3D.GetAttachmentModel(i);
      ...
      INDEX iNumHl3      = PlayerHealth%10;
		  ...
		  if (m_fBorderHealth > _tmNow) { iHealthBorderAlpha = 255/(m_h3dAppearTime/fHudAppearDiff); }
		  else { m_h3dAppearTime = 0; }
      ...
      FLOAT fHudAppearDiffArmor = m_fBorderArmor - _tmNow;
		  if (m_fBorderArmor > _tmNow) { iBrdArmr = 255/(m_h3dAppearTimeArmor/fHudAppearDiffArmor); }
		  else { m_h3dAppearTimeArmor = 0; }
      ...
      INDEX iNumMn1      = (PlayerMana%1000000)/100000;// 100000
      INDEX iNumMn2      = (PlayerMana%100000)/10000;  // 010000
      INDEX iNumMn3      = (PlayerMana%10000)/1000;    // 001000
      INDEX iNumMn4      = (PlayerMana%1000)/100;      // 000100
      INDEX iNumMn5      = (PlayerMana%100)/10;        // 000010
      INDEX iNumMn6      =  PlayerMana%10;             // 000001
      ...
      if (PlayerHealth < 10)   {iNumHl2 = 11;}
		  ...
		  icohlt.mo_colBlendColor  = h3d_iColor|255;
		  /*if (PlayerHealth <= 25)  {icohlt.mo_colBlendColor  = 0xFF000000|255;} else {icohlt.mo_colBlendColor  = h3d_iColor|255;}*/
	
      CModelObject &dgthlt1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_001_DGTHLT100)->amo_moModelObject; dgthlt1.mo_toTexture.PlayAnim(iNumHl1+100, 0); dgthlt1.mo_colBlendColor  = h3d_iColor|255;
      CModelObject &dgthlt2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_002_DGTHLT010)->amo_moModelObject; dgthlt2.mo_toTexture.PlayAnim(iNumHl2+100, 0); dgthlt2.mo_colBlendColor  = h3d_iColor|255;
      CModelObject &dgthlt3 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_003_DGTHLT001)->amo_moModelObject; dgthlt3.mo_toTexture.PlayAnim(iNumHl3+100, 0); dgthlt3.mo_colBlendColor  = h3d_iColor|255;
      CModelObject &brdhlth = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_102_BRDHLTH  )->amo_moModelObject; brdhlth.mo_toTexture.PlayAnim(98, 0);      brdhlth.mo_colBlendColor  = 0xFF000000|iHealthBorderAlpha;
      ...
      if (PlayerArmor <= 0  ) {iIcoArr = 15;}  //icon armor
		  ...
		  icoarr.mo_toTexture.PlayAnim(iIcoArr, AOF_LOOPING|AOF_NORESTART); icoarr.mo_colBlendColor  = h3d_iColor|255;
		  /*if (PlayerArmor <= 25) {icoarr.mo_colBlendColor  = 0xFF000000|255;} else {icoarr.mo_colBlendColor  = h3d_iColor|255;}*/

      CModelObject &dgtarr1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_005_DGTARR100)->amo_moModelObject; dgtarr1.mo_toTexture.PlayAnim(iNumAr1+100, 0); dgtarr1.mo_colBlendColor  = h3d_iColor|255;
      CModelObject &dgtarr2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_006_DGTARR010)->amo_moModelObject; dgtarr2.mo_toTexture.PlayAnim(iNumAr2+100, 0); dgtarr2.mo_colBlendColor  = h3d_iColor|255;
      CModelObject &dgtarr3 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_007_DGTARR001)->amo_moModelObject; dgtarr3.mo_toTexture.PlayAnim(iNumAr3+100, 0); dgtarr3.mo_colBlendColor  = h3d_iColor|255;
		  CModelObject &brdarmr = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_103_BRDARMR  )->amo_moModelObject; brdarmr.mo_toTexture.PlayAnim(98, 0);      brdarmr.mo_colBlendColor  = 0xFF000000|iBrdArmr;
      ...
      if (GetSP()->sp_bSinglePlayer || GetSP()->sp_bCooperative) {
        CModelObject &icoscr  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_008_ICOSCR      )->amo_moModelObject;  icoscr.mo_toTexture.PlayAnim(iIcoFrg, 0); icoscr.mo_colBlendColor  = h3d_iColor|255;

        CModelObject &dgtscr1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_009_DGTSCR100000)->amo_moModelObject; dgtscr1.mo_toTexture.PlayAnim(iNumSc1+100, 0);
        CModelObject &dgtscr2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_010_DGTSCR010000)->amo_moModelObject; dgtscr2.mo_toTexture.PlayAnim(iNumSc2+100, 0);
        CModelObject &dgtscr3 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_011_DGTSCR001000)->amo_moModelObject; dgtscr3.mo_toTexture.PlayAnim(iNumSc3+100, 0);
        CModelObject &dgtscr4 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_012_DGTSCR000100)->amo_moModelObject; dgtscr4.mo_toTexture.PlayAnim(iNumSc4+100, 0);
        CModelObject &dgtscr5 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_013_DGTSCR000010)->amo_moModelObject; dgtscr5.mo_toTexture.PlayAnim(iNumSc5+100, 0);
        CModelObject &dgtscr6 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_014_DGTSCR000001)->amo_moModelObject; dgtscr6.mo_toTexture.PlayAnim(iNumSc6+100, 0);
      }
      if (GetSP()->sp_gmGameMode == CSessionProperties::GM_FRAGMATCH) {
        CModelObject &icoscr  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_008_ICOSCR      )->amo_moModelObject; icoscr.mo_toTexture.PlayAnim(iIcoFrg, 0); icoscr.mo_colBlendColor  = h3d_iColor|255;

        CModelObject &dgtscr1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_009_DGTSCR100000)->amo_moModelObject; dgtscr1.mo_toTexture.PlayAnim(iNumFr1+100, 0);
        CModelObject &dgtscr2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_010_DGTSCR010000)->amo_moModelObject; dgtscr2.mo_toTexture.PlayAnim(iNumFr2+100, 0);
        CModelObject &dgtscr3 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_011_DGTSCR001000)->amo_moModelObject; dgtscr3.mo_toTexture.PlayAnim(iNumFr3+100, 0);
        CModelObject &dgtscr4 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_012_DGTSCR000100)->amo_moModelObject; dgtscr4.mo_toTexture.PlayAnim(iNumFr4+100, 0);
        CModelObject &dgtscr5 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_013_DGTSCR000010)->amo_moModelObject; dgtscr5.mo_toTexture.PlayAnim(iNumFr5+100, 0);
        CModelObject &dgtscr6 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_014_DGTSCR000001)->amo_moModelObject; dgtscr6.mo_toTexture.PlayAnim(iNumFr6+100, 0);
      }
      if (GetSP()->sp_gmGameMode == CSessionProperties::GM_SCOREMATCH) {
        CModelObject &icoscr  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_008_ICOSCR      )->amo_moModelObject;  icoscr.mo_toTexture.PlayAnim(iIcoScr, 0); icoscr.mo_colBlendColor  = h3d_iColor|255;

        CModelObject &dgtscr1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_009_DGTSCR100000)->amo_moModelObject; dgtscr1.mo_toTexture.PlayAnim(iNumDmSc1+100, 0);
        CModelObject &dgtscr2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_010_DGTSCR010000)->amo_moModelObject; dgtscr2.mo_toTexture.PlayAnim(iNumDmSc2+100, 0);
        CModelObject &dgtscr3 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_011_DGTSCR001000)->amo_moModelObject; dgtscr3.mo_toTexture.PlayAnim(iNumDmSc3+100, 0);
        CModelObject &dgtscr4 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_012_DGTSCR000100)->amo_moModelObject; dgtscr4.mo_toTexture.PlayAnim(iNumDmSc4+100, 0);
        CModelObject &dgtscr5 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_013_DGTSCR000010)->amo_moModelObject; dgtscr5.mo_toTexture.PlayAnim(iNumDmSc5+100, 0);
        CModelObject &dgtscr6 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_014_DGTSCR000001)->amo_moModelObject; dgtscr6.mo_toTexture.PlayAnim(iNumDmSc6+100, 0);
      }
      ...
      if (GetSP()->sp_gmGameMode == CSessionProperties::GM_FRAGMATCH) {
		  CModelObject &icoskl  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_015_ICOSKL      )->amo_moModelObject;  icoskl.mo_toTexture.PlayAnim(iIcoDth, 0); icoskl.mo_colBlendColor  = h3d_iColor|255;

      CModelObject &dgtdth1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_016_DGTDTH100000)->amo_moModelObject; dgtdth1.mo_toTexture.PlayAnim(iNumDt1+100, 0); dgtdth1.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_017_DGTDTH010000)->amo_moModelObject; dgtdth2.mo_toTexture.PlayAnim(iNumDt2+100, 0); dgtdth2.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth3 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_018_DGTDTH001000)->amo_moModelObject; dgtdth3.mo_toTexture.PlayAnim(iNumDt3+100, 0); dgtdth3.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth4 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_019_DGTDTH000100)->amo_moModelObject; dgtdth4.mo_toTexture.PlayAnim(iNumDt4+100, 0); dgtdth4.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth5 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_020_DGTDTH000010)->amo_moModelObject; dgtdth5.mo_toTexture.PlayAnim(iNumDt5+100, 0); dgtdth5.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth6 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_021_DGTDTH000001)->amo_moModelObject; dgtdth6.mo_toTexture.PlayAnim(iNumDt6+100, 0); dgtdth6.mo_colBlendColor  = H3DC_WHITE|255;
      } else if (GetSP()->sp_gmGameMode == CSessionProperties::GM_SCOREMATCH) {
      CModelObject &icoskl  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_015_ICOSKL      )->amo_moModelObject;  icoskl.mo_toTexture.PlayAnim(iIcoDth, 0); icoskl.mo_colBlendColor  = h3d_iColor|255;

      CModelObject &dgtdth1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_016_DGTDTH100000)->amo_moModelObject; dgtdth1.mo_toTexture.PlayAnim(iNumMn1+100, 0); dgtdth1.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_017_DGTDTH010000)->amo_moModelObject; dgtdth2.mo_toTexture.PlayAnim(iNumMn2+100, 0); dgtdth2.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth3 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_018_DGTDTH001000)->amo_moModelObject; dgtdth3.mo_toTexture.PlayAnim(iNumMn3+100, 0); dgtdth3.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth4 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_019_DGTDTH000100)->amo_moModelObject; dgtdth4.mo_toTexture.PlayAnim(iNumMn4+100, 0); dgtdth4.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth5 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_020_DGTDTH000010)->amo_moModelObject; dgtdth5.mo_toTexture.PlayAnim(iNumMn5+100, 0); dgtdth5.mo_colBlendColor  = H3DC_WHITE|255;
      CModelObject &dgtdth6 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_021_DGTDTH000001)->amo_moModelObject; dgtdth6.mo_toTexture.PlayAnim(iNumMn6+100, 0); dgtdth6.mo_colBlendColor  = H3DC_WHITE|255;
      }
      ...
      CModelObject &icopusd  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_022_ICOPWRUPSD  )->amo_moModelObject;  icopusd.mo_toTexture.PlayAnim(iIcoPUSD, AOF_LOOPING|AOF_NORESTART); icopusd.mo_colBlendColor  = 0xFF520000|255;
      CModelObject &dgtpusd1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_023_DGTPWRUPSD10)->amo_moModelObject; dgtpusd1.mo_toTexture.PlayAnim(iNumPUSD1+100, 0); dgtpusd1.mo_colBlendColor  = 0xFF520000|255;
      CModelObject &dgtpusd2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_024_DGTPWRUPSD01)->amo_moModelObject; dgtpusd2.mo_toTexture.PlayAnim(iNumPUSD2+100, 0); dgtpusd2.mo_colBlendColor  = 0xFF520000|255;
      ...
      CModelObject &icopuiu  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_025_ICOPWRUPIU  )->amo_moModelObject;  icopuiu.mo_toTexture.PlayAnim(iIcoPUIU, AOF_LOOPING|AOF_NORESTART); icopuiu.mo_colBlendColor  = 0x40D0FF00|255;
      CModelObject &dgtpuiu1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_026_DGTPWRUPIU10)->amo_moModelObject; dgtpuiu1.mo_toTexture.PlayAnim(iNumPUIU1+100, 0); dgtpuiu1.mo_colBlendColor  = 0x40D0FF00|255;
      CModelObject &dgtpuiu2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_027_DGTPWRUPIU01)->amo_moModelObject; dgtpuiu2.mo_toTexture.PlayAnim(iNumPUIU2+100, 0); dgtpuiu2.mo_colBlendColor  = 0x40D0FF00|255;
      ...
      CModelObject &icopuss  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_028_ICOPWRUPSS  )->amo_moModelObject;  icopuss.mo_toTexture.PlayAnim(iIcoPUSS, AOF_LOOPING|AOF_NORESTART); icopuss.mo_colBlendColor  = 0xFFB20000|255;
      CModelObject &dgtpuss1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_029_DGTPWRUPSS10)->amo_moModelObject; dgtpuss1.mo_toTexture.PlayAnim(iNumPUSS1+100, 0); dgtpuss1.mo_colBlendColor  = 0xFFB20000|255;
      CModelObject &dgtpuss2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_030_DGTPWRUPSS01)->amo_moModelObject; dgtpuss2.mo_toTexture.PlayAnim(iNumPUSS2+100, 0); dgtpuss2.mo_colBlendColor  = 0xFFB20000|255;
      ...
      CModelObject &icopuis  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_031_ICOPWRUPIS  )->amo_moModelObject;  icopuis.mo_toTexture.PlayAnim(iIcoPUIS, AOF_LOOPING|AOF_NORESTART); icopuis.mo_colBlendColor  = 0xCCCCCC00|255;
      CModelObject &dgtpuis1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_032_DGTPWRUPIS10)->amo_moModelObject; dgtpuis1.mo_toTexture.PlayAnim(iNumPUIS1+100, 0); dgtpuis1.mo_colBlendColor  = 0xCCCCCC00|255;
      CModelObject &dgtpuis2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_033_DGTPWRUPIS01)->amo_moModelObject; dgtpuis2.mo_toTexture.PlayAnim(iNumPUIS2+100, 0); dgtpuis2.mo_colBlendColor  = 0xCCCCCC00|255;
      ...
      if (!GetSP()->sp_bInfiniteAmmo) {
		  ...
		  CModelObject &brdammo  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_104_BRDAMMO     )->amo_moModelObject; brdammo.mo_toTexture.PlayAnim(98, /*AOF_LOOPING|AOF_NORESTART*/ 0);  brdammo.mo_colBlendColor  = 0xFF000000|iBrdAmmo;
      ...
      if (GetSP()->sp_bSinglePlayer || GetSP()->sp_bCooperative) {
        ...
        if (fTimeLeft <= 30) {
		  ...
		  CModelObject &icomsg  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_037_ICOMSG       )->amo_moModelObject;  icomsg.mo_toTexture.PlayAnim(iIcoMsg, AOF_LOOPING|AOF_NORESTART); icomsg.mo_colBlendColor  = h3d_iColor|iLowTimer;
		  CModelObject &dgtmsg1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_038_DGTMSG100    )->amo_moModelObject; dgtmsg1.mo_toTexture.PlayAnim(iNumMsg1, 0); dgtmsg1.mo_colBlendColor  = h3d_iColor|iLowTimer;
		  CModelObject &dgtmsg2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_039_DGTMSG010    )->amo_moModelObject; dgtmsg2.mo_toTexture.PlayAnim(iNumMsg2, 0); dgtmsg2.mo_colBlendColor  = h3d_iColor|iLowTimer;
		  CModelObject &dgtmsg3 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_040_DGTMSG001    )->amo_moModelObject; dgtmsg3.mo_toTexture.PlayAnim(iNumMsg3, 0); dgtmsg3.mo_colBlendColor  = h3d_iColor|iLowTimer;
      ...
      if (iCountType == 1)  {iIcoCnt  = 14;} else {iIcoCnt  = 13;}
		  ...
		  CModelObject &icocnt  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_041_ICOBSSHLT    )->amo_moModelObject;  icocnt.mo_toTexture.PlayAnim(iIcoCnt,  0);  icocnt.mo_colBlendColor  = h3d_iColor|255;
		  CModelObject &numcnt1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_042_DGTBSSHLT100 )->amo_moModelObject; numcnt1.mo_toTexture.PlayAnim(iNumCnt1+112, 0); numcnt1.mo_colBlendColor  = h3d_iColor|255;
		  CModelObject &numcnt2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_043_DGTBSSHLT010 )->amo_moModelObject; numcnt2.mo_toTexture.PlayAnim(iNumCnt2+112, 0); numcnt2.mo_colBlendColor  = h3d_iColor|255;
		  CModelObject &numcnt3 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_044_DGTBSSHLT001 )->amo_moModelObject; numcnt3.mo_toTexture.PlayAnim(iNumCnt3+112, 0); numcnt3.mo_colBlendColor  = h3d_iColor|255;
      ...
      if (iOxygen < 10) {iNumOxy1 = 11;}
		  ...
		  CModelObject &icooxy  = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_045_ICOOXYGEN    )->amo_moModelObject;  icooxy.mo_toTexture.PlayAnim(iIcoOxy, AOF_LOOPING|AOF_NORESTART); icooxy.mo_colBlendColor  = h3d_iColor|255;
		  CModelObject &dgtoxy1 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_046_DGTOXYGEN10  )->amo_moModelObject; dgtoxy1.mo_toTexture.PlayAnim(iNumOxy1+112, 0); dgtoxy1.mo_colBlendColor  = h3d_iColor|255;
		  CModelObject &dgtoxy2 = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_047_DGTOXYGEN01  )->amo_moModelObject; dgtoxy2.mo_toTexture.PlayAnim(iNumOxy2+112, 0); dgtoxy2.mo_colBlendColor  = h3d_iColor|255;
      ...
      if (fGetAmmIrb >  0.1 ) {iBarIrb  = 62;}
		  ...
		  CModelObject &icosrb = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_072_ICOAMSRBM     )->amo_moModelObject; icosrb.mo_toTexture.PlayAnim(iIcoSrb, 0); icosrb.mo_colBlendColor  = h3d_iColor|255;
		  CModelObject &barsrb = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_073_BARAMSRBM     )->amo_moModelObject; barsrb.mo_toTexture.PlayAnim(iBarSrb, 0); barsrb.mo_colBlendColor  = h3d_iColor|255;
      ...
      if (iAvbWep & (1<<(WEAPON_KNIFE-1)))           {iAvbWepKnf = 69;} else {iAvbWepKnf = 68;} if (iWntWep ==  1) {iWntWepKnf = 97;} else {iWntWepKnf = 96;}
		  ...
		  CModelObject &icoknf = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_074_ICOWPKNIF    )->amo_moModelObject; icoknf.mo_toTexture.PlayAnim(iAvbWepKnf, 0); icoknf.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdknf = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_075_BRDWPKNIF    )->amo_moModelObject; brdknf.mo_toTexture.PlayAnim(iWntWepKnf, 0); brdknf.mo_colBlendColor  = H3DC_WHITE|DisWep;        
		  CModelObject &icochs = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_076_ICOWPCHSW    )->amo_moModelObject; icochs.mo_toTexture.PlayAnim(iAvbWepChs, 0); icochs.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdchs = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_077_BRDWPCHSW    )->amo_moModelObject; brdchs.mo_toTexture.PlayAnim(iWntWepChs, 0); brdchs.mo_colBlendColor  = H3DC_WHITE|DisWep;        
		  CModelObject &icoclt = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_078_ICOWPCOLT    )->amo_moModelObject; icoclt.mo_toTexture.PlayAnim(iAvbWepClt, 0); icoclt.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdclt = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_079_BRDWPCOLT    )->amo_moModelObject; brdclt.mo_toTexture.PlayAnim(iWntWepClt, 0); brdclt.mo_colBlendColor  = H3DC_WHITE|DisWep;        
		  CModelObject &icodcl = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_080_ICOWPDBCL    )->amo_moModelObject; icodcl.mo_toTexture.PlayAnim(iAvbWepDcl, 0); icodcl.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brddcl = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_081_BRDWPDBCL    )->amo_moModelObject; brddcl.mo_toTexture.PlayAnim(iWntWepDcl, 0); brddcl.mo_colBlendColor  = H3DC_WHITE|DisWep;        
		  CModelObject &icosht = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_082_ICOWPSHGN    )->amo_moModelObject; icosht.mo_toTexture.PlayAnim(iAvbWepSht, 0); icosht.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdsht = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_083_BRDWPSHGN    )->amo_moModelObject; brdsht.mo_toTexture.PlayAnim(iWntWepSht, 0); brdsht.mo_colBlendColor  = H3DC_WHITE|DisWep;        
		  CModelObject &icodsh = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_084_ICOWPDBSG    )->amo_moModelObject; icodsh.mo_toTexture.PlayAnim(iAvbWepDsh, 0); icodsh.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brddsh = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_085_BRDWPDBSG    )->amo_moModelObject; brddsh.mo_toTexture.PlayAnim(iWntWepDsh, 0); brddsh.mo_colBlendColor  = H3DC_WHITE|DisWep;        
		  CModelObject &icotmg = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_086_ICOWPTMGN    )->amo_moModelObject; icotmg.mo_toTexture.PlayAnim(iAvbWepTmg, 0); icotmg.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdtmg = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_087_BRDWPTMGN    )->amo_moModelObject; brdtmg.mo_toTexture.PlayAnim(iWntWepTmg, 0); brdtmg.mo_colBlendColor  = H3DC_WHITE|DisWep;        
		  CModelObject &icomgn = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_088_ICOWPMNGN    )->amo_moModelObject; icomgn.mo_toTexture.PlayAnim(iAvbWepMgn, 0); icomgn.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdmgn = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_089_BRDWPMNGN    )->amo_moModelObject; brdmgn.mo_toTexture.PlayAnim(iWntWepMgn, 0); brdmgn.mo_colBlendColor  = H3DC_WHITE|DisWep;
		  CModelObject &icorkl = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_090_ICOWPRKLN    )->amo_moModelObject; icorkl.mo_toTexture.PlayAnim(iAvbWepRkl, 0); icorkl.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdrkl = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_091_BRDWPRKLN    )->amo_moModelObject; brdrkl.mo_toTexture.PlayAnim(iWntWepRkl, 0); brdrkl.mo_colBlendColor  = H3DC_WHITE|DisWep;
		  CModelObject &icogrl = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_092_ICOWPGRLN    )->amo_moModelObject; icogrl.mo_toTexture.PlayAnim(iAvbWepGrl, 0); icogrl.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdgrl = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_093_BRDWPGRLN    )->amo_moModelObject; brdgrl.mo_toTexture.PlayAnim(iWntWepGrl, 0); brdgrl.mo_colBlendColor  = H3DC_WHITE|DisWep;
		  CModelObject &icoflm = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_094_ICOWPFLMR    )->amo_moModelObject; icoflm.mo_toTexture.PlayAnim(iAvbWepFlm, 0); icoflm.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdflm = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_095_BRDWPFLMR    )->amo_moModelObject; brdflm.mo_toTexture.PlayAnim(iWntWepFlm, 0); brdflm.mo_colBlendColor  = H3DC_WHITE|DisWep;
		  CModelObject &icosnp = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_096_ICOWPSNPR    )->amo_moModelObject; icosnp.mo_toTexture.PlayAnim(iAvbWepSnp, 0); icosnp.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdsnp = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_097_BRDWPSNPR    )->amo_moModelObject; brdsnp.mo_toTexture.PlayAnim(iWntWepSnp, 0); brdsnp.mo_colBlendColor  = H3DC_WHITE|DisWep;
		  CModelObject &icolsr = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_098_ICOWPLSER    )->amo_moModelObject; icolsr.mo_toTexture.PlayAnim(iAvbWepLsr, 0); icolsr.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdlsr = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_099_BRDWPLSER    )->amo_moModelObject; brdlsr.mo_toTexture.PlayAnim(iWntWepLsr, 0); brdlsr.mo_colBlendColor  = H3DC_WHITE|DisWep;
		  CModelObject &icocnn = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_100_ICOWPCNON    )->amo_moModelObject; icocnn.mo_toTexture.PlayAnim(iAvbWepCnn, 0); icocnn.mo_colBlendColor  = h3d_iColor|DisWep;
		  CModelObject &brdcnn = m_moH3D.GetAttachmentModel(H3D_BASE_ATTACHMENT_101_BRDWPCNON    )->amo_moModelObject; brdcnn.mo_toTexture.PlayAnim(iWntWepCnn, 0); brdcnn.mo_colBlendColor  = H3DC_WHITE|DisWep;
      if (iShls <= 0) {icosht.mo_colBlendColor = H3DC_DIS|DisWep; icodsh.mo_colBlendColor = H3DC_DIS|DisWep;}
		  if (iShls >  0) {icosht.mo_colBlendColor = h3d_iColor|DisWep; icodsh.mo_colBlendColor = h3d_iColor|DisWep;}
		  if (iBlts <= 0) {icotmg.mo_colBlendColor = H3DC_DIS|DisWep; icomgn.mo_colBlendColor = H3DC_DIS|DisWep;}
		  if (iBlts >  0) {icotmg.mo_colBlendColor = h3d_iColor|DisWep; icomgn.mo_colBlendColor = h3d_iColor|DisWep;}
		  if (iRckt <= 0) {icorkl.mo_colBlendColor = H3DC_DIS|DisWep;}
		  if (iRckt >  0) {icorkl.mo_colBlendColor = h3d_iColor|DisWep;}
      if (iGrnd <= 0) {icogrl.mo_colBlendColor = H3DC_DIS|DisWep;}
		  if (iGrnd >  0) {icogrl.mo_colBlendColor = h3d_iColor|DisWep;}
		  if (iNplm <= 0) {icoflm.mo_colBlendColor = H3DC_DIS|DisWep;}
		  if (iNplm >  0) {icoflm.mo_colBlendColor = h3d_iColor|DisWep;}
		  if (iSnbl <= 0) {icosnp.mo_colBlendColor = H3DC_DIS|DisWep;}
		  if (iSnbl >  0) {icosnp.mo_colBlendColor = h3d_iColor|DisWep;}
		  if (iElec <= 0) {icolsr.mo_colBlendColor = H3DC_DIS|DisWep;}
		  if (iElec >  0) {icolsr.mo_colBlendColor = h3d_iColor|DisWep;}
		  if (iIrbl <= 0) {icocnn.mo_colBlendColor = H3DC_DIS|DisWep;}
		  if (iIrbl >  0) {icocnn.mo_colBlendColor = h3d_iColor|DisWep;}
